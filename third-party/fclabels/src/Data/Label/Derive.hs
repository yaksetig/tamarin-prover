 ...
 do -- Only process data and newtype declarations, filter out all
    ...
    let (name, cons, vars) =
          case dec of
            ...
            -- GHC 9.8+: TyVarBndr gained a BndrVis parameter to
            -- carry the type-variable visibility annotation.  The
            -- rest of fclabels still expects [TyVarBndr ()], so
            -- drop the visibility tag right at the boundary.
            DataD    _ n vs _ cs _ -> (n, cs,  map (() <$) vs)
            NewtypeD _ n vs _ c  _ -> (n, [c], map (() <$) vs)
